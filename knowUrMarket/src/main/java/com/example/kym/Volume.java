...
public class Volume extends ActionBarActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(s.contentEquals("E"))
			setContentView(R.layout.volosc);
		else if(s.contentEquals("H"))
		{
			setContentView(R.layout.volosch);
		}
		else if(s.contentEquals("B"))
		{
			setContentView(R.layout.voloscb);
		}
		getSupportActionBar().setBackgroundDrawable(new ColorDrawable(Color.parseColor("#3b5999")));
		getSupportActionBar().setTitle("Volume Oscillator");
	}
	@Override
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		{
			Intent j=new Intent(getApplicationContext(), Volume.class);
			startActivity(j);
		}
	}
}
